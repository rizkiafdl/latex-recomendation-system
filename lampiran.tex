\begin{center}
    \Large\bfseries LAMPIRAN
\end{center}
\vspace{1cm}

% === LAMPIRAN 1 ===
\begin{center}
    % Menampilkan gambar surat terlebih dahulu
    % Tinggi disetel 0.73\textheight agar ruang halaman cukup untuk judul LAMPIRAN di atas dan label di bawah
    \includegraphics[height=0.73\textheight]{pic/suratsurvey.jpg}
    
    \vspace{0.5cm}
    % Keterangan ditaruh di bawah gambar dengan format miring (italic) sesuai referensi
    \textit{\large Lampiran 1 Surat Survei Mahasiswa}
\end{center}
\newpage

% === LAMPIRAN 2 ===
\noindent\textbf{\large Lampiran 2: Tautan Repository dan Dokumen Pendukung}
\vspace{0.4cm}

Berikut adalah tautan-tautan yang berkaitan dengan penelitian ini, meliputi repositori kode sumber, dokumen skripsi, serta instrumen pengumpulan data. [cite: 1657]
\vspace{0.3cm}

\begin{table}[htbp]
\centering
\renewcommand{\thetable}{2.1} % Penomoran otomatis menjadi Tabel 2.1 mengikuti Lampiran 2
\caption{Daftar Tautan Pendukung Penelitian}
\label{tab:tautan_pendukung}
\vspace{0.3cm}

\small
\begin{tabular}{|c|p{4.5cm}|p{8.5cm}|}
\hline
\textbf{No.} & \textbf{Keterangan} & \textbf{Tautan} \\ \hline

1 & Repository Kode Sumber Sistem Rekomendasi & 
\raggedright \texttt{https://github.com/\allowbreak rizkiafdl/\allowbreak faculty-recommender-thesis} \tabularnewline \hline

2 & Repository Dokumen Skripsi (LaTeX) & 
\raggedright \texttt{https://github.com/\allowbreak rizkiafdl/\allowbreak latex-recomendation-system} \tabularnewline \hline

3 & Google Form Wawancara & 
\raggedright \texttt{https://docs.google.com/\allowbreak forms/d/e/\allowbreak 1FAIpQLSfWDj3e4Xc\allowbreak 2C01WKA1sZ678JEMajcQ\allowbreak 6110aUIOmKWGRgJYQ/\allowbreak viewform?usp=dialog} \tabularnewline \hline

4 & Spreadsheet Hasil Wawancara & 
\raggedright \texttt{https://docs.google.com/\allowbreak spreadsheets/d/\allowbreak 12wOS4fbvfyNWXB5pDLm-\allowbreak ZomiBE5gsTlkulVt\allowbreak U1cvs/edit?usp=sharing} \tabularnewline \hline

\end{tabular}
\end{table}

\newpage

% === PENGATURAN WARNA & BANNER KUSTOM ===
\definecolor{binusblue}{rgb}{0.0, 0.45, 0.74}

\newcommand{\cvsection}[1]{%
    \vspace{0.2cm}
    \noindent
    \begingroup
    \setlength{\arrayrulewidth}{1.2pt}%
    \arrayrulecolor{binusblue}%
    \begin{tabular}{|p{14.5cm}|}
        \hline
        \vspace{-0.1cm}\textbf{\color{binusblue}\large #1}\vspace{0.05cm} \\
        \hline
    \end{tabular}
    \endgroup
    \vspace{0.15cm}
}

\begin{singlespace}
\vspace{0.2cm}
\begin{center}
    \textbf{\large RIWAYAT HIDUP}
\end{center}
\vspace{0.15cm}

\begin{center}
    \includegraphics[width=2cm]{pic/rizkiafdoli_ijazah.png}
\end{center}
\vspace{0.15cm}

% === PERSONAL INFORMATION ===
\cvsection{PERSONAL INFORMATION}

\noindent
\begin{tabular}{p{3.5cm} p{0.2cm} p{10.5cm}}
Binusian ID       & : & 2602139141 \\
Full Name         & : & Muhammad Rizki Afdolli \\
E-Mail            & : & muhammad.afdolli@binus.ac.id \\
Address           & : & Perumahan Taman Pulomas BLOK B2, NO 11, Kedungjaya, Kec. Kedawung, Kabupaten Cirebon, Jawa Barat 45153 \\
Mobile            & : & +6281394269183 \\
Gender            & : & Male \\
Birth Race / Date & : & Jawa / Cirebon, 03/05/2004 \\
Nationality       & : & Indonesia \\
Marital Status    & : & Single \\
Religion          & : & Islam \\
\end{tabular}

% === FORMAL EDUCATION ===
\cvsection{FORMAL EDUCATION}

\noindent
\begin{tabular}{p{4.5cm} p{9.5cm}}
2022 -- 2026 & \textbf{BINUS University} \\
             & School of Computer Science -- Teknik Informatika \\
             & GPA : 3.64 / 4.00 \\
2019 -- 2022 & \textbf{SMAN 1 KOTA CIREBON} \\
             & IPA \\
\end{tabular}

% === PERSONAL CERTIFICATION ===
\cvsection{PERSONAL CERTIFICATION}

\noindent
\begin{tabular}{p{4.5cm} p{9.5cm}}
2025 & \textbf{Astronomer Certification for Apache Airflow 3 Fundamentals} \\
2024 & \textbf{AWS Academy Graduate - AWS Academy Cloud Architecting} \\
2024 & \textbf{AWS Academy Graduate - AWS Academy Cloud Foundations} \\
\end{tabular}

% === ORGANIZATION EXPERIENCE ===
\cvsection{ORGANIZATION EXPERIENCE}

\noindent
\begin{tabular}{p{4.5cm} p{9.5cm}}
2024 -- 2025 & \textbf{Front End Mentor at BNCC} \\
2024 -- 2025 & \textbf{Data Science Core Team at GDSC Binus} \\
2023 -- 2024 & \textbf{Staff Of EEO at BNCC} \\
\end{tabular}

% === WORKING EXPERIENCE ===
\cvsection{WORKING EXPERIENCE}

\noindent
\begin{tabular}{p{4.5cm} p{9.5cm}}
2026 -- Present & \textbf{DANA Indonesia - Data Quality Engineer} \\
2025 -- 2026 & \textbf{99 Group - Data Engineer} \\
\end{tabular}
\end{singlespace}

% ============================================================
% RIWAYAT HIDUP — PERSON 2
% Salin blok ini untuk orang berikutnya. Jangan tambah \definecolor atau \newcommand lagi.
% ============================================================
\newpage

\begin{singlespace}
\vspace{0.2cm}
\begin{center}
    \textbf{\large RIWAYAT HIDUP}
\end{center}
\vspace{0.15cm}

\begin{center}
    \includegraphics[width=2cm]{pic/rakha_photo.jpg}
\end{center}
\vspace{0.15cm}

\cvsection{PERSONAL INFORMATION}

\noindent
\begin{tabular}{p{3.5cm} p{0.2cm} p{10.5cm}}
Binusian ID       & : & 2602187241 \\
Full Name         & : & Rakha Naufal Azizi \\
E-Mail            & : & rakha.azizi@binus.ac.id \\
Address           & : & Jl. Keranji No.19A, Bukit Sari, Kec. Gerunggang, Kota Pangkal Pinang, Kepulauan Bangka Belitung 33123 \\
Mobile            & : & +6285273889013 \\
Gender            & : & Male \\
Birth Race / Date & : & PangkalPinang, 28/07/2004 \\
Nationality       & : & Indonesia \\
Marital Status    & : & Single \\
Religion          & : & Islam \\
\end{tabular}

\cvsection{FORMAL EDUCATION}

\noindent
\begin{tabular}{p{4.5cm} p{9.5cm}}
2022 -- 2026 & \textbf{BINUS University} \\
             & School of Computer Science -- Teknik Informatika \\
             & GPA : 3.61 / 4.00 \\
2019 -- 2022 & \textbf{SMKN 2 PangkalPinang} \\
             & Teknik Komputer dan Jaringan \\
\end{tabular}

\cvsection{PERSONAL CERTIFICATION}

\noindent
\begin{tabular}{p{4.5cm} p{9.5cm}}
- & \textbf{-} \\
\end{tabular}

\cvsection{ORGANIZATION EXPERIENCE}

\noindent
\begin{tabular}{p{4.5cm} p{9.5cm}}
2023 -- 2024 & \textbf{Staff Of RnD at BNCC} \\
2022 -- 2023 & \textbf{Activist Of LnT at BNCC} \\
2022 -- 2023 & \textbf{Activist at MCB Binus @Bandung} \\
\end{tabular}

\cvsection{WORKING EXPERIENCE}

\noindent
\begin{tabular}{p{4.5cm} p{9.5cm}}
2025 -- 2026 & \textbf{Bina Nusantara University - IT Developer} \\
2025 -- 2026 & \textbf{PT. Telkom Witel Babel - Data Management} \\
\end{tabular}
\end{singlespace}

% ============================================================
% RIWAYAT HIDUP — PERSON 3
% Salin blok ini untuk orang berikutnya. Jangan tambah \definecolor atau \newcommand lagi.
% ============================================================
\newpage

\begin{singlespace}
\vspace{0.2cm}
\begin{center}
    \textbf{\large RIWAYAT HIDUP}
\end{center}
\vspace{0.15cm}

\begin{center}
    \includegraphics[width=2cm]{pic/fototheo.jpg}
\end{center}
\vspace{0.15cm}

\cvsection{PERSONAL INFORMATION}

\noindent
\begin{tabular}{p{3.5cm} p{0.2cm} p{10.5cm}}
Binusian ID       & : & 2602096230 \\
Full Name         & : & Theofilus Adhi Septian \\
E-Mail            & : & theofilus.septian@binus.ac.id \\
Address           & : & Green Valley Residence no 31 Bandung \\
Mobile            & : & 082129501795 \\
Gender            & : & Male \\
Birth Race / Date & : & Sunda / Bandung , 22/09/2004 \\
Nationality       & : & Indonesia \\
Marital Status    & : & Single \\
Religion          & : & Katolik \\
\end{tabular}

\cvsection{FORMAL EDUCATION}

\noindent
\begin{tabular}{p{4.5cm} p{9.5cm}}
2022 -- 2026 & \textbf{BINUS University} \\
             & School of Computer Science -- Teknik Informatika \\
             & GPA : 3.36 / 4.00 \\
2019 -- 2022 & \textbf{SMAS Santa Angela BANDUNG} \\
             & IPA \\
\end{tabular}

\cvsection{PERSONAL CERTIFICATION}

\noindent
\begin{tabular}{p{4.5cm} p{9.5cm}}
2026 & \textbf{C++ Intermediate - Sololearn} \\
2026 & \textbf{Python Intermediate - Sololearn} \\
2026 & \textbf{Introduction to Python - Sololearn} \\
2023 & \textbf{Introduction to C - Sololearn} \\
\end{tabular}

\cvsection{ORGANIZATION EXPERIENCE}

\noindent
\begin{tabular}{p{4.5cm} p{9.5cm}}

2023 -- 2024 & \textbf{CEO at Bina Nusantara Computer Club @Bandung} \\
2022 -- 2023 & \textbf{Aktivis at KMK Binus @Bandung} \\
\end{tabular}

\cvsection{WORKING EXPERIENCE}

\noindent
\begin{tabular}{p{4.5cm} p{9.5cm}}
2025 -- Present & \textbf{WINGS Group -- Data \& CRM Specialist} \\
\end{tabular}
\end{singlespace}